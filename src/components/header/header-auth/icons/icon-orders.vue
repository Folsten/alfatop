<template>
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="20"
    height="20"
    viewBox="0 0 20 20"
    fill="none"
  >
    <g clip-path="url(#clip0_2_9836)">
      <path
        d="M10.8335 4.16669H17.5002"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M10.8335 7.50003H15.0002"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M10.8335 12.5H17.5002"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M10.8335 15.8334H15.0002"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M2.5 4.16671C2.5 3.94569 2.5878 3.73373 2.74408 3.57745C2.90036 3.42117 3.11232 3.33337 3.33333 3.33337H6.66667C6.88768 3.33337 7.09964 3.42117 7.25592 3.57745C7.4122 3.73373 7.5 3.94569 7.5 4.16671V7.50004C7.5 7.72105 7.4122 7.93302 7.25592 8.0893C7.09964 8.24558 6.88768 8.33337 6.66667 8.33337H3.33333C3.11232 8.33337 2.90036 8.24558 2.74408 8.0893C2.5878 7.93302 2.5 7.72105 2.5 7.50004V4.16671Z"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M2.5 12.5C2.5 12.279 2.5878 12.067 2.74408 11.9108C2.90036 11.7545 3.11232 11.6667 3.33333 11.6667H6.66667C6.88768 11.6667 7.09964 11.7545 7.25592 11.9108C7.4122 12.067 7.5 12.279 7.5 12.5V15.8334C7.5 16.0544 7.4122 16.2663 7.25592 16.4226C7.09964 16.5789 6.88768 16.6667 6.66667 16.6667H3.33333C3.11232 16.6667 2.90036 16.5789 2.74408 16.4226C2.5878 16.2663 2.5 16.0544 2.5 15.8334V12.5Z"
        stroke="#344054"
        stroke-width="1.66667"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
    </g>
    <defs>
      <clipPath id="clip0_2_9836">
        <rect
          width="20"
          height="20"
          fill="white"
        />
      </clipPath>
    </defs>
  </svg>
</template>